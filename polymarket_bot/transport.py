...
lock, so one slow reply holds back every other one.
...
Orders do not go this way: the knock library (knocker.py) sends them over
its own HTTP/2 connections. What is left here is cancels, reconciliation
reads, exits and signing lookups: small, bursty at market end, never
hundreds deep.
"""
...
# The per-phase timeout (connect, write, read-gap) of the pool below.
WORST_REPLY_SECONDS = 1.0
SYNC_POOL_CONNECTIONS = 64
# Dials the warm-up makes ahead of a knock, for the cancels right after the
# door.
WARM_CONNECTIONS = 3
# A market handed back by signing re-enters placement every loop tick, and
